_base_ = ["../../models/swin/swin_tiny.py", "../../models/default_runtime.py"]
model = dict(
    backbone=dict(patch_size=(2, 4, 4), drop_path_rate=0.1), test_cfg=dict(max_testing_views=4)
)
print(model)
# dataset settings
...
test_pipeline = [
    dict(type="DecordInit"),
    dict(type="SampleFrames", clip_len=4, frame_interval=2, num_clips=4, test_mode=True),
    dict(type="DecordDecode"),
...
# learning policy
lr_config = dict(
    policy="CosineAnnealing", min_lr=0, warmup="linear", warmup_by_epoch=True, warmup_iters=2.5
)
total_epochs = 20
