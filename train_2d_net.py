...
class ResNetClassifier(pl.LightningModule):
    ...
    def train_dataloader(self):
        ...
        self.train_dataset.dataset.transforms.transform = transform
    ...
    def val_dataloader(self):
        ...
        train_len = int(0.8*len(img_train))
        val_len = len(img_train) - train_len

        self.train_dataset, self.val_dataset = torch.utils.data.random_split(img_train, [train_len, val_len])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-tr", "--transfer",
                        ...
                        action="store_true")
    parser.add_argument("-to", "--tune_fc_only", help="Tune only the final, fully connected layers.",
                        action="store_true")
    ...
    # # Instantiate Model
    dataset_root = 'action_dataset/imgs'
    model = ResNetClassifier(num_classes=15, resnet_version=18,
                             train_path=dataset_root, test_path=None,
                             optimizer='adam', lr=1e-3,
                             batch_size=32, transfer=True, tune_fc_only=False)
    # Instantiate lightning trainer and train model
    checkpoint_callback = pl.callbacks.ModelCheckpoint(dirpath='2d/')
    trainer_args = {'gpus': None, 'max_epochs': 100, "callbacks": [checkpoint_callback]}
